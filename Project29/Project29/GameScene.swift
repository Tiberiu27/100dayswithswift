...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    var windType: String? {
        didSet {
            windLabel.text = "\(windType ?? "no") wind comming from \(windDirection ?? "nowhere")"
        }
    }
    ...
    func destroy(player: SKSpriteNode) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 2) {
            //challenge 1
            if self.viewController.player2Score == 3 || self.viewController.player1Score == 3 {
    ...
    func showWindLabel() {
        ...
        windLabel.position = CGPoint(x: 512, y: 648)
        windLabel.text = "No wind comming from nowhere"
        windLabel.fontSize = 20
        windLabel.fontColor = .white
        addChild(windLabel)
        
        switch CGFloat.random(in: -4...4) {
        case 2.5...4:
            windDirection = "left"
            windType = "Strong"
            physicsWorld.gravity = CGVector(dx: CGFloat.random(in: 2.5...4), dy: -9.8)
        case -2.5...2.5:
            windDirection = "left or right"
            windType = "Fair"
            physicsWorld.gravity = CGVector(dx: CGFloat.random(in: -2.5...2.5), dy: -9.8)
        default:
            windDirection = "right"
            windType = "Strong"
            physicsWorld.gravity = CGVector(dx: CGFloat.random(in: -4...2.5), dy: -9.8)
        }
