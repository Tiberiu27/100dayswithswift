...
class SelectionViewController: UITableViewController {
    ...
    func saveToChache(name: String) -> UIImage? {
        ...
        if let tempItems = try? fm.contentsOfDirectory(atPath: Bundle.main.resourcePath!) {
            for item in tempItems {
                if item.range(of: "Large") != nil {
                    items.append(item)
                    //load from cache
                    if let image = saveToChache(name: item) {
                        images.append(image)
                    } else {
                        //create and save to cache
                        images.append(createImages(currentImage: item)!)
                    }
                }
            }
        }
    ...
    func createImages(currentImage: String) -> UIImage? {
